...
```{r plot_t_2}
chart(data = biometry, age_rec1 ~ rapport_tour_taille_taille)) +
  geom_boxplot(na.rm = TRUE) +
  ylab("Rapport tour taille/taille") +
  xlab("Groupe d'âge") +
  ggtitle("Distribution du rapport tour de taille/taille par groupe d'âge")
```

<!--% Hypothèse nulle et alternative -->

moyenne rapport - moyenne rapport = 0
...
```{r test_t_2}
  #àutiliser pour anova car 4niveua au lieu de deux
t.test(data=biometry, rapport_tour_taille_taille ~ age_rec, altenative="two.sided", conf.level=0.95, var.equal=TRUE)
```
