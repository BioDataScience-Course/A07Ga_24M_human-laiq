...
# Calcul de la ou des variables reprenant l'indice sous forme numérique (et facteur)
biometry %>.%
  smutate(.,  rapport_tour_taille_taille = tour_taille/taille )
```
...
bio_tab <- table(biometry$age_rec, biometry$bmi)
tabularise(bio_tab)
```
...
H0 : moyenne(bmiF) - moyenne(bmiH) = 0

H1 : moyenne(bmiF) - moyenne(bmiH) != 0

<!-- Vérification des conditions d'application -->

```{r test_t_1}
t.test(data = biometry, bmi ~ genre, alternative = "two.sided", conf.level =0.95, var.equal = FALSE)
```
...
```{r plot_t_2}
## Recodage de biometry$age en biometry$age_rec2
biometry$age_rec2 <- cut(biometry$age,
  include.lowest = TRUE,
  right = FALSE,
  dig.lab = 4,
  breaks = c(16, 30, 45, 60, 85)
)
chart(data = biometry, masse_corr~biometry$age_rec2) +
  geom_boxplot(na.rm=TRUE)+
  ylab("Masse en kilogrammes")+
  xlab("Age")
```
