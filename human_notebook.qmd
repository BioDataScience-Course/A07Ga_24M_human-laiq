...
```{r}
chart(data = biometry, imc ~ fast_food_rec %col=% genre %group=% genre) +
    geom_line() +
    geom_point()
```
...
Les résultats de notre test, ne nous permettent pas de rejeter H0. Il n'y a donc pas d'effet significatif du genre ou de la consommation de fast-food sur l'imc au seuil de 5%. Il n'y a pas d'interaction entre le genre et le fast-food.
